...
7. 若原技师不可用，只有在槽位中存在明确“可以换”的 `NO_PREFERENCE`/替代许可时才查询替代候选，不擅自改变服务时长或预算。候选排序返回事实理由，用户选定一个后才创建单个短期占位。
8. 展示确认卡片：门店、明确日期、开始/结束时间、项目、技师、价格、规则、占位截止时间。生成绑定方案版本的确认入口；自然语言“好”不直接等于授权。
9. 用户通过可信确认接口确认，服务端记录确认事件并生成业务授权凭据，执行幂等提交。
10. 同一事务中锁定并校验任务、授权、占位和依赖事实，按 §8.2 的有效报价策略确认可履约；更新占用为 BOOKED、写订单、记录操作结果、写 Outbox 和审计。
11. 事务成功后返回订单卡片，后台异步发送提醒。天气若有可信结果可附加，失败则只省略该段。
...
| search_availability | 只读 | 候选资源、时间、约束满足情况、snapshot_at |
...
统一结果字段：status、data、error_code、retryable、operation_id、observed_at、schema_version。错误枚举区分 VALIDATION_ERROR、PERMISSION_DENIED、SLOT_CONFLICT、HOLD_EXPIRED、STALE_PROPOSAL、VERSION_CONFLICT、DEPENDENCY_UNAVAILABLE、UNKNOWN_OUTCOME，以及 IDEMPOTENCY_MISMATCH、NOT_FOUND、RATE_LIMITED、CONFIRMATION_REQUIRED、HUMAN_TAKEOVER_ACTIVE、LEASE_LOST。对无权访问的对象可统一为 NOT_FOUND，避免泄漏存在性。
...
先执行硬过滤：租户/门店、合法服务、技能、营业和班次、资源容量、预算、用户明确禁选项；再做排序。起步可用归一化分数：技能匹配 0.35、显式偏好 0.25、时间贴合 0.20、价格贴合 0.10、可用评价 0.10。它只是可解释基线，缺失特征需重新归一化，不能虚构评价。

排序结果返回分数组成和证据，LLM 只解释。通过用户选择率、任务成功率、改约率、取消率和不同技师曝光分布评估，不能仅优化点击率。若生成式解释没有带来可重复的业务收益，保留确定性排序和服务端事实作为候选生成核心，不能让模型替代规则。
