...
export function useMessages(
	...
	options?: {
		...
		/** Called when the durable handoff state changes on this session. */
		onHandoffUpdated?: (record: HandoffRecord) => void;
		/** Called when a persisted human handoff message is published. */
		onHandoffMessage?: (message: HandoffMessage) => void;
		/** Called for a durable refund state event from replay/live SSE. */
		onRefundStateChanged?: (event: RefundOperationStateEvent) => void;
	},
) {
	...
	// Reply ids already loaded as complete history. The SSE endpoint replays
	// its bounded event log before switching to live delivery; without this
	// guard those old deltas would be appended to the same persisted reply a
	// second time after a page refresh.
	const completedReplyIdsRef = useRef<Set<string>>(new Set());
	...
	const processEvent = useCallback(
		(event: AgentEvent) => {
			// Durable refund events use the same replay/live transport but are
			// business-state messages, not AgentScope reply events.
			const businessEvent = event as AgentEvent &
				Partial<RefundOperationStateEvent>;
			if (
				businessEvent.event_type === 'refund_operation_state_changed' &&
				typeof businessEvent.operation_id === 'string' &&
				typeof businessEvent.state_version === 'number'
			) {
				optionsRef.current?.onRefundStateChanged?.(
					businessEvent as RefundOperationStateEvent,
				);
				return;
			}
			...
			if (event.type === EventType.CUSTOM) {
				...
				} else if (custom.name === 'handoff_reply_intercepted') {
					// The old bot reply remains persisted for audit/staff use, but
					// it is no longer a customer-visible answer once human takeover
					// wins the run revision race.
					const value = custom.value as { reply_id?: string };
					if (value.reply_id) {
						msgsRef.current = msgsRef.current.filter(
							(message) => message.id !== value.reply_id,
						);
						completedReplyIdsRef.current.add(value.reply_id);
					}
					currentReplyRef.current = null;
					clearInterruptTimer();
					setPhase('idle');
				} else if (custom.name === 'handoff_run_intercepted') {
					// Covers a takeover that wins before REPLY_START is delivered.
					currentReplyRef.current = null;
					clearInterruptTimer();
					setPhase('idle');
				} else if (custom.name === 'handoff_state_changed' && custom.value) {
					const value = custom.value as { handoff?: HandoffRecord };
					if (value.handoff) optionsRef.current?.onHandoffUpdated?.(value.handoff);
				} else if (custom.name === 'handoff_message' && custom.value) {
					const value = custom.value as { message?: HandoffMessage };
					if (value.message) optionsRef.current?.onHandoffMessage?.(value.message);
			...
			const replyId = 'reply_id' in event ? event.reply_id : undefined;
			if (
				typeof replyId === 'string' &&
				completedReplyIdsRef.current.has(replyId)
			) {
				return;
			}
			if (event.type === EventType.REPLY_START) {
				...
				if (event.type === EventType.REPLY_END) {
					if (typeof replyId === 'string') {
						completedReplyIdsRef.current.add(replyId);
					}
					clearInterruptTimer();
					setPhase('idle');
					currentReplyRef.current = null;
	...
	useEffect(() => {
		...
		msgsRef.current = [];
		currentReplyRef.current = null;
		completedReplyIdsRef.current = new Set();
		setMsgs([]);
		setError(null);
		clearInterruptTimer();
		setPhase('idle');
		...
		(async () => {
			...
			if (takeFreshlyCreated(sessionId)) {
				...
				try {
					...
					if (cancelled) return;
					// A superseded bot reply is retained server-side for audit and
					// the staff workspace, but must not reappear in the customer's
					// transcript after a refresh.
					const visibleMessages = messages.filter(
						(message) => !message.metadata?.handoff_intercepted,
					);
					msgsRef.current = visibleMessages;
					completedReplyIdsRef.current = new Set(
						visibleMessages
							.filter((message) => message.role === 'assistant' && message.finished_at)
