...
import { handoffApi, refundApi, sessionApi } from '@/api';
...
export function ChatViewport({ agentId, sessionId, onSessionsChanged }: ChatViewportProps) {
	...
	const [refundStatus, setRefundStatus] = useState<RefundOperationStatus | null>(null);
	...
	useEffect(() => {
		...
		void (async () => {
			try {
				const record = await handoffApi.get(sessionId, agentId);
				if (active) setHandoffStatus(record.status);
			} catch {
				// A session without a handoff record is the normal initial state.
			}
			try {
				const response = await handoffApi.messages(sessionId, agentId);
				if (active) setHandoffMessages(response.items);
			} catch {
				// The scoped endpoint deliberately returns 404 when no handoff exists.
			}
			// The durable status endpoint is the source of truth after refresh
			// or a lost event. A session without an operation is normal.
			try {
				const response = await refundApi.sessionStatus(sessionId);
				const latest = response.items[0];
				if (active && latest) setRefundStatus(latest);
			} catch {
				// No operation or a scoped 404 is expected for most sessions.
			}
		})();
		return () => {
			active = false;
	...
	const handleRequestHandoff = useCallback(async () => {
		if (!sessionId || !agentId || handoffPending) return;
		setHandoffPending(true);
		try {
			const record = await handoffApi.request(sessionId, agentId);
			setHandoffStatus(record.status);
		} catch {
			// The shared client presents the server's controlled error.
		} finally {
			setHandoffPending(false);
		}
	}, [sessionId, agentId, handoffPending]);

	const handleHandoffUpdated = useCallback((record: HandoffRecord) => {
		setHandoffStatus(record.status);
		setHandoffPending(false);
	}, []);

	const handleHandoffMessage = useCallback((message: HandoffMessage) => {
	...
	const handleRefundStateChanged = useCallback((event: RefundOperationStateEvent) => {
		setRefundStatus((current) => {
			if (current && event.state_version <= current.state_version) return current;
			return {
				...(current ?? {
		...
		// Fill the compact SSE projection from the durable endpoint. This
		// handles a refresh or an event that arrived before the initial read.
		void refundApi
			.status(event.operation_id)
			.then((status) => {
				setRefundStatus((current) =>
					current && status.state_version < current.state_version
						? current
						: status,
				);
			})
			.catch(() => {
				// The event itself remains useful; a later replay/status read can
				// recover the full projection.
			});
	}, []);
	...
	const {
		...
		onTeamUpdated: handleTeamUpdated,
		 ...
		 onHandoffMessage: handleHandoffMessage,
		 onRefundStateChanged: handleRefundStateChanged,
		});
	const {
		mcps,
		loading: mcpsLoading,
							...
							{refundStatus && (
								<Badge variant="outline" className="self-center">
									...
									>
										...
										{handoffStatus === 'human_active'
											? '人工处理中'
											: handoffStatus === 'queued' || handoffPending
												? '转人工中'
												: '转人工'}
									</Button>
									<LlmSelect
									...
									<DropdownMenu>
										...
										{handoffMessages.length > 0 && (
											<div className="mx-auto mb-2 w-full max-w-[var(--chat-content-w)] rounded-lg border bg-muted/30 p-3 text-sm">
												<div className="mb-2 font-medium">人工客服消息</div>
												<div className="space-y-2">
													{handoffMessages.map((message) => (
														<div key={message.message_id} className="rounded-md bg-background p-2">
															<div className="mb-1 text-xs text-muted-foreground">
																{message.sender_role === 'staff' ? '坐席' : '客户'} ·{' '}
